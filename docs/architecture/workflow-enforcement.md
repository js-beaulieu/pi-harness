...
- Knowledge writes go through `workspace_knowledge`; they are allowed only in `plan` and `code`.
...
- The parent Pi session is treated as an orchestrator and cannot edit `projects/**`. Only `pi-subagents` children in the `impl` or `impl-lite` role receive editing tools.
