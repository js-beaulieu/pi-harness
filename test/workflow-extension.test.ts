...
test("one adjustable plan approval processes multiple projects in order", async () => workspace(async (root) => {
  ...
  const h = harness(); await h.commands.get("workspace:knowledge-backfill").handler("api tasks-api", ctx(root)); assert.match(h.messages[0].message, /history_start once/); assert.match(h.messages[0].message, /Stop all tools while waiting/); assert.match(h.messages[0].message, /output:"backfill-handoff\.json", outputMode:"file-only"/); assert.match(h.messages[0].message, /Do not set turnBudget or toolBudget/); const blockedPlanningAnalysis = await h.events.get("tool_call")({ toolName: "mcp:codebase-memory/search", input: {} }, ctx(root)); assert.equal(blockedPlanningAnalysis.block, true); assert.match(blockedPlanningAnalysis.reason, /metadata-only/); const knowledge = h.tools.get("workspace_knowledge"); assert.equal("planToken" in knowledge.parameters.properties, false); assert.equal("planTokens" in knowledge.parameters.properties, false); assert.equal("runToken" in knowledge.parameters.properties, false); assert.equal("complexity" in knowledge.parameters.properties, false); const impact = h.tools.get("workspace_knowledge_impact"); const record = h.tools.get("workspace_knowledge_tree_record"); const apiPlan = await knowledge.execute("id", { action: "history_plan", project: "api" }, undefined, undefined, ctx(root)); const tasksPlan = await knowledge.execute("id", { action: "history_plan", project: "tasks-api" }, undefined, undefined, ctx(root)); const started = await knowledge.execute("id", { action: "history_start", projects: ["api", "tasks-api"] }, undefined, undefined, ctx(root)); assert.deepEqual(started.details.projects.map((project: any) => project.project), ["api", "tasks-api"]); assert.match(started.content[0].text, /Adjust adjacent boundaries/); const blockedWhileWaiting = await h.events.get("tool_call")({ toolName: "bash", input: { command: "git log --oneline" } }, ctx(root)); assert.equal(blockedWhileWaiting.block, true); assert.match(blockedWhileWaiting.reason, /waiting for the user's/); assert.equal(await h.events.get("tool_call")({ toolName: "ask_user", input: {} }, ctx(root)), undefined); h.asks.get("ask:answered")({ context: `pi-harness:backfill-start:${started.details.approvalKey}`, response: { kind: "selection", selections: ["Start processing"] } }); const blockedDuringProcessing = await h.events.get("tool_call")({ toolName: "bash", input: { command: "git grep service" } }, ctx(root)); assert.equal(blockedDuringProcessing.block, true); assert.match(blockedDuringProcessing.reason, /Parent shell exploration is disabled during backfill processing/); const allowedHandoffRead = await h.events.get("tool_call")({ toolName: "read", input: { path: "backfill-handoff.json" } }, ctx(root)); assert.equal(allowedHandoffRead, undefined);
