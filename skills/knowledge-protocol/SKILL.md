...
For each returned segment, launch exactly one `backfill` agent with the `scopeToken` and all segment evidence using `async:true`, `clarify:false`, `context:"fresh"`, `artifacts:false`, `output:"backfill-handoff.json"`, `outputMode:"file-only"`, and no cwd override. The backfill agent queries the code graph, calls `workspace_knowledge_impact` with the `scopeToken` and graph result, inspects the source, and writes its complete structured handoff to the output file. Do not set `turnBudget` or `toolBudget` — the read-only backfill agent must run to completion. Immediately `wait` once for that run; never poll status. The child is the only component that uses read-only Git/grep and the code graph for detailed segment analysis.

After the run completes, read the `backfill-handoff.json` output file and pass its parsed JSON to `workspace_knowledge_tree_record` once. The `impactToken` in the handoff is a short opaque hex string generated by the backfill agent's `workspace_knowledge_impact` call — copy it exactly as-is. Use `nodes: []` only when all required dispositions establish no durable knowledge. Record the segment before calling `history_chunk` again with no additional inputs, and report the resulting guidance paths so progress is visible in `docs/knowledge/`.
...
After all history segments are recorded, `current_scope` is mandatory for each project. Call it to get the `scopeToken` and complete existing-node facts, then launch one `backfill` agent with the same `async:true`, `output:"backfill-handoff.json"`, `outputMode:"file-only"` configuration used for history segments. The backfill agent queries the graph, calls `workspace_knowledge_impact`, checks every existing node against current behavior, and classifies every current file. Read the handoff and call `workspace_knowledge_tree_record`. Every current code file must be covered by an active node and will appear explicitly in the generated code-file ownership index; `no-durable-knowledge` is allowed only for non-code files with a factual reason. For an updated current node, supply its complete current claim set so obsolete claims are removed. Retire nodes whose behavior no longer exists. Only a map marked **Current snapshot: validated** is actionable for future plans, features, and bug fixes.
